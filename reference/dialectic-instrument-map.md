...
| Phase 1              | `focus-interview` when no confirmed focus is inherited; `elenchus`; `third-pole`; conditional `frame-projector`; `home-frame-leak`; prepare `neutral-control` | Focus and deeper stakes confirmed; third-pole result recorded even when none is found; frame-projector shows the typed phenomenology, candidate clusters, tested separators, label workshop, and several complete ASCII/box-drawing projections with relevant examples, or records the specific contraindication; blind reconnaissance passed its leak check or carries its named downgrade; neutral pre-belief baseline frozen. |
