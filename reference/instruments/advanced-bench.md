...
- **Phenomenon / range / input:** Dominant clusters in ambiguous, high-dimensional phenomenology; candidate separating lines; evocative prototypes; and missing quadrants. Needs a bounded question or territory plus enough observed, sourced, elicited, or explicitly generated items to cluster. Avoid when the material is plainly one-dimensional, statistically tractable, first-principles clear, or better represented as a taxonomy.
- **Unaided limit / access differential:** Starting from favored axes hides whether the boxes describe real clusters. Rao's cluster-first projection makes the phenomenology, candidate groupings, separating lines, naming choices, occupied cells, and empty cells inspectable in sequence.
- **Perturbation / procedure:** Follow the full [Rao sequence](../quadrant-diagrams.md#rao-sequence): inventory and type the phenomenology; form candidate four-cluster partitions before axes; discover separating lines; workshop evocative labels in the appropriate naming mode; render and repopulate each diagram; reject or revise maps that fail the examples, chemistry, or independence checks. Generate several honest candidates without choosing a winning frame.
- **Readout / control:** Show the phenomenology inventory, candidate clusters and members, candidate separators and failures, label workshop, rendered maps, and calibration in that order. Render every candidate as a complete inline ASCII/box-drawing 2×2 under [the rendering contract](../quadrant-diagrams.md#rendering-contract): both named axes and poles, visible directions, four bounded labeled cells, 1–2 relevant examples in every occupied cell or explicit empty marks, source pointers, axis claim type, second-axis confidence, and the axis-independence result. A polished diagram without its formation trace—or a one-line `axis × axis: four labels` list, bullets, or table—is an incomplete readout. Retain an honest contraindication when the material does not support a quadrant diagram.
- **Common artifacts:** Axes chosen before looking at the phenomenology; clusters reverse-engineered to fill boxes; false orthogonality or symmetry; witty labels detached from members; generated examples mistaken for evidence; and an empty quadrant mistaken for a solution.
...
- **Readout / control:** Terse residual items with source pointers, the lens that exposed each item, evidence status, and whether the omission came from the active frame or missing input. Do not explain the residue's importance or fold it into a new frame.
- **Common artifacts:** The orchestrator treats personal interest as signal or invents residue to keep the inquiry open.
- **Escalate / stop:** Offer another instrument when a residual item maps to its measurement target. Stop when another remainder pass yields no new sourced item.
- **Cost / persistence:** One analytical pass; session on a Walk, log or tension pages in larger apparatus.
- **Execution placement:** **Either; orchestrator by default.** The executor sees the structured reading and its source trace; a fresh subagent may receive the same material without the orchestrator's preferred result for a separated remainder scan. Separation improves control but is not required. If only the orchestrator runs it, state capture risk. Return the residual items as a separate list; do not integrate them.
...
- **Perturbation / procedure:** Generate synthesis, juxtaposition, ground-condition, framing-dissolution, or undecidable-centered candidate samples using [Phase 5](../phase5-sublation.md). Label every output as generated material.
