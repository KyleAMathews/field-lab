...
- [Focus interview](#focus-interview-focus-interview)
- [Substrate map](#substrate-map-substrate-map)
- [Stake map](#stake-map-stake-map)
- [Term scan](#term-scan-term-scan)
- [Tension statement](#tension-statement-tension-statement)
- [Third-pole probe](#third-pole-probe-third-pole)
- [Ground-condition probe](#ground-condition-probe-ground-condition)
- [Small experiment](#small-experiment-small-experiment)
...
- **Unaided limit / access differential:** The initial prompt shows what the user chose to state, not which part is live, tacit, doubtful, or consequential. Responsive questions make that hidden context available before analysis hardens around the first frame.
- **Operating range:** Use before substantive work on interpretive, strategic, personal, creative, evaluative, dialectical, or aim-dependent practical questions. This includes fact-shaped requests whose answer may vary by intended outcome, named method, specimen state, or constraints. Skip stable facts, narrow mechanical tasks, explicit one-shot requests, and cases where prior conversation already supplies current answers.
...
- **Procedure:** List actors or parts, events, order, timing, conditions, handoffs, and the point at which someone declares completion. For a planned intervention, include the current state, retained state, removal or change, likely response, and next management step. Mark missing observations.
- **Readout:** A short “what happens now” sequence plus two or three unknowns.
- **Control:** Separate direct observation from report and inference.
...
- **Unaided limit / access differential:** Proposed solutions hide the costs they answer. Structured elicitation separates reported stakes, inferred stakes, and fixed constraints by person.
- **Operating range:** Coordination, preferences, personal choices, norms, and relationship friction. Do not use to diagnose a person or invent hidden motives.
- **Input:** The affected people and one concrete point of friction.
- **Perturbation:** Direct attention from stated solutions to the cost each person experiences.
- **Procedure:** For each person, distinguish feelings, desired outcome, standard, constraint, feared loss, and priority. Ask for missing testimony rather than fill it in.
- **Readout:** A compact map of aligned stakes, conflicting stakes, and unknown stakes.
- **Control:** Mark user testimony separately from inference; offer inferred stakes as questions.
- **Common artifact:** The orchestrator supplies a polished motive the person never endorsed.
- **Escalate when:** Two valid stakes cannot fit together, or a repeated word hides different standards.
- **Stop when:** Each person can recognize the map and a small experiment is available.
- **Cost / persistence:** 1–4 turns; session only.
- **Execution placement:** **Orchestrator.** It keeps direct access to the user's testimony and marks inferred stakes as questions. A subagent may re-scan an existing transcript only as a separate control, not replace elicitation. If live testimony is unavailable, return a provisional map with gaps. The orchestrator presents it for recognition or correction.
...
- **Phenomenon sought:** The smallest contradiction that explains recurring friction.
- **Unaided limit / access differential:** A list of complaints does not show which demands collide under which condition. Compression makes one candidate mechanism testable.
...
- **Input:** At least two claims or stakes and the condition that makes them collide.
- **Perturbation:** Compresses a messy field into a candidate structure.
- **Procedure:** State: “We need or value **A**, and we also need or value **B**, but under **C**, doing A undermines B because **mechanism**.” Then search for omitted facts, actors, and axes.
- **Readout:** One or two candidate tensions, each with disconfirming evidence that would weaken it.
...
- **Procedure:** Ask who rejects both poles, what they optimize instead, and whether that view has its own constituency, practice, evidence, or literature. Prefer an orthogonal axis.
- **Readout:** A qualified third pole with its independent basis, or a clear “none found.”
- **Control:** Test whether the candidate is reachable by sliding between A and B. If yes, reject it.
