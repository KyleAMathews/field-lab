...
| Phase 1              | `focus-interview` when no confirmed focus is inherited; `elenchus`; `third-pole`; `frame-projector`; `home-frame-leak`; prepare `neutral-control` | Focus and deeper stakes confirmed; third-pole result recorded even when none is found; several honest frame projections compared; blind reconnaissance passed its leak check or carries its named downgrade; neutral pre-belief baseline frozen. |
...
| Phase 3              | complete `belief-stress`; compare `neutral-control`                                                                                               | Independent committed positions returned; hedging and decorrelation checked; probe-induced additions separated from the neutral baseline; essays and readout traces persisted.                                                                   |
| Phase 4A             | `fracture-scan`; `residue-collect`; `frame-projector`                                                                                             | One immanent fracture per position; shared assumptions and protected interests preserved as residue; hidden-question projection is honest or recorded as one-dimensional.                                                                        |
