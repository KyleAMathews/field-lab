...
**Reversibility check (Boyd).** Trace each major claim back to specific atomic parts from the decomposition. If a claim doesn't trace back, flag it — confabulation or unsupported new insight. Check the "not the same arrangement" test: structurally, is S just one monk's architecture wearing the other's vocabulary? That fails.
