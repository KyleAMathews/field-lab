...
The OODA loop adds a clear division of authority. In camera mode, the human owns **Observe and Orient**. The agent asks for missing substrate, offers instruments, and returns bounded readings from instruments the user selects. It does not decide what the readings mean as a whole and must not complete **Decide and Act** on the user's behalf.
...
3. **Offer.** Present three materially distinct instruments with what each measures and why its reading may help. Wait for the user's selection; a calling signal alone is not authorization.
