...
4. **Rank and finalize (orchestrator).** Pick the final 3–5 functional donors maximizing structural fit × conceptual density, under a hard diversity rule: **no two donors from the same broad field.** If two patterns' best match lands in the same field (e.g. both in life sciences), the second takes its strongest donor from a different field. Monoculture is a failure mode — a sea of anarchy that is all biology inherits biology's priors (selection, fitness, organism boundaries) and the diversity is illusory.
