...
2. **Assemble a problem-agnostic diversity menu.** ~25–30 maximally varied fields spanning the breadth of human knowledge — life sciences, physical sciences, earth/climate systems, mathematics, engineering, economics, law, linguistics, religion/ritual, visual & performing arts, craft, history, and so on. Generate it fresh each round (optionally seed a few entries from random Wikipedia categories to fight habit). Build it *without reference to the problem* — it is a general palette, not a shortlist for this dialectic. The menu is the diversity-enforcement mechanism: spread is guaranteed by construction.
3. **Dispatch the blind recruiter** with ONLY the structural briefs + the menu (no problem, no home domain, no monk essays). For each pattern it picks the 2–3 menu fields whose core abstractions best match, names the specific sub-concept (not "biology" but "endosymbiosis / CoRR"), and gives one line on the structural match. Enforced ignorance — it cannot drift to adjacent fields because it never sees the home domain and is choosing from a deliberately broad palette.
4. **Rank and finalize (orchestrator) — hard no-clustering rule.** Pick the final 3–5 functional donors to **span as many unrelated meta-domains as possible** — living systems, physical systems, social/institutional, formal/mathematical, linguistic/cultural, artistic. Require **≥3 distinct meta-domains; never two donors from the same one.** This rule binds *you, the selector*, because the conceptual-habit bias re-enters precisely here: you will be tempted to collapse the set onto the meta-domain whose mechanisms read as most *legible and rich* to you (for many orchestrators, biology). **That legibility IS your habit** — the exact filter the blind recruiter just worked to defeat, sneaking back one step downstream at selection. The recruiter will have offered spread (linguistics, law, folklore, mycology, ritual…); do not quietly drop the less-legible ones. If two patterns' best match share a meta-domain, the second takes its strongest donor from a different one, even at some cost to per-pattern fit. Monoculture is a failure mode: a sea that is all one meta-domain inherits that domain's deep priors (e.g. biology's selection / fitness / organism-boundary) and its diversity is illusory.
...
2. **Construct the sea of anarchy.** First write a **domain manifest** — an explicit list of every domain in the pool as peers: `[Monk A, Monk B, (Monk C…), donor1, donor2, donor3…]` (donors recruited in 4.5b). Every entry on the manifest gets decomposed to the **same depth** — this is the guard against the monk-primary habit of shattering the essays thoroughly and skimming the donors. Then **list the atomic components** of every domain on the manifest — claims, mechanisms, evidence, assumptions, metaphors, principles, operations — stripped of which monk or donor said them.
   ...
   **Tag each part with a calibration estimate.** For monk-derived parts: independent of how confidently the monk asserted it, is this part well-supported (cite-backable, mechanistically sound) or a rhetorical reach? Mark each `[solid]` / `[plausible]` / `[reach]`. The monks argue at full conviction by design; calibration is *per-part*, and the synthesis weights by it (Phase 5), not by rhetorical force. **For donor-derived parts, calibration tracks a different axis — isomorphism strength to the problem, not truth in the home field.** A donor fact can be textbook-solid biology yet a `[reach]` *as applied*; tag the strength of the structural match, or every richly-researched donor waves through as `[solid]` and the tag does no work. And once the operations pass (step 3) produces donor↔monk recombinations, **calibrate those too** — the recombination, not the raw part, is where the synthesis risk actually lives.
