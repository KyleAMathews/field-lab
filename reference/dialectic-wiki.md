...
| **Observation ledger**     | append-only                                       | Findings carried from Walk, Survey, research, and instruments with their kinds (`observation`, `user-testimony`, `source-claim`, `elicited-response`, `inference`, `analogy`, `normative-judgment`, `hypothesis`), support, confidence, and artifact risk. Later analysis may transform claims but not erase provenance.                                                                                                                                                                                   |
| **Instrument ledger**      | append-only; lifecycle state may advance          | One entry per instrument run or prepared cross-phase run: call, lifecycle state, actual execution seat and contexts, fallback, access delta, typed findings, control, artifact risk, residue, trace paths, user-feedback state, and caddy result. Preparation never masquerades as a completed reading. The phase gate cites these entries.                                                                                                                                                                |
