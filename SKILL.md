...
Stable facts, narrow mechanical work, explicit one-shot requests, and urgent safety precautions may bypass camera mode. A request for a dialectic—including “the output is a dialectic”—selects the full dialectic workflow and authorizes its named outputs, including candidate construction and synthesis where earned; it does not authorize unrelated decisions or actions. Reserve standalone short Monks for users who explicitly ask for a quick, short, lightweight, or sketch version. The brake applies on Walks, Field Trips, Expeditions, and inside workflows.

## Default stance: take a Walk

- Outside a tutorial or the entry contract of a workflow the user has selected, do not announce a mode, recite a process, or ask the user to choose a workflow. The required just-in-time instrument handshake is not a mode or workflow announcement. If the host requires a skill-use announcement, name the skill without predicting a compact or full route before reading the specimen.
- If the user explicitly asks to learn or try the field lab, run a tutorial session. This is an exception to the rule against process explanation: briefly explain Walks, opening a field log for a Field Trip, Expeditions that collect related trips, independent workflows, camera mode, and engine mode; then offer three small, low-stakes exercises whose instruments produce clearly different readings. Let the user choose, teach one instrument at a time, and keep the normal selection, handshake, raw-readout, and interpretation boundaries. Do not create artifacts unless the user asks.
...
- Run an instrument only after the user selects it. A direct request for a named instrument counts as selection. An agreed Field Trip plan or named workflow schedule selects only its listed instruments; creating an Expedition selects none. Any extra ad hoc instrument needs a new choice. The automatic focus interview is the sole exception: asking the questions prepares the reading, and the user authorizes completion by answering.
...
- After each useful readout, keep the orientation exits open: pause; offer another probe; let the user reframe the specimen; offer to open a field log; or offer a fitting workflow. Run, decide, or act only after the corresponding explicit selection or engine transition.
...
- A user reply that selects a named workflow ends the general caddy choice. Enter that workflow directly and let its schedule, instruments, and checkpoints govern the next move. Do not make the user select the workflow twice. Selection loads the schedule; it does not authorize an unattended phase run. Orient the user to the workflow, open the current phase, and wait for the phase-start go-ahead required by that workflow.
...
A suggestion, calling signal, or newly noticed pattern is not authorization to run an instrument or leave orientation. The three choices are a small field kit for the user, not a dump of the registry; never invent a weak option merely to reach three. If the user's response to the preceding readout selects a named workflow, skip this gate and hand off to that workflow.
...
The number and strength of instruments do not define these forms. Opening a log adds memory; an Expedition adds cross-trip navigation; neither removes instruments or selects a workflow. Walk instruments remain available everywhere and each unscheduled run still needs selection.

Workflows are a separate layer. A workflow coordinates steps, instruments, controls, people, artifacts, and gates toward a named kind of reading. The Electric Monk dialectic is one workflow; future workflows may be added without creating another field-work state. Camera versus engine is another independent axis: all field work defaults to feedback-rich camera mode.
...
3. **Offer or hand off.** Present three fitting instruments when the user has not selected a next operation. If the user selected a named workflow in the focus response, enter it directly; do not insert another scale or instrument menu.
...
7. **Offer the next exposure.** Pause; offer probes; let the user reframe; offer to open a field log; offer an Expedition for related trips; or offer a fitting workflow. Cross into conclusion, decision, or action only through the OODA orientation gate.
8. **Materialize explicitly.** Never open a field log, start an Expedition, or select a workflow as a quiet side effect of a Walk.
...
Treat a request for a “dialectic,” or a focus response such as “the output is dialectic,” as selection of the full Electric Monk dialectic workflow. After the focus interview supplies the needed substrate, acknowledge the selection and begin the workflow entry contract: give the short seven-phase orientation, explain the Field Trip and field-log artifacts, show the Phase 1 opening card, then stop for the user's go-ahead before creating artifacts or running Phase 1. Do not offer a second scale choice. Offer standalone short blind Monks only when the user explicitly asks for a quick, short, lightweight, or sketch treatment. Requests for a hostile thesis test, strongest case on each side, determinate negation, or validation still require context-isolated positions at minimum; when “dialectic” is not named, ask only if the desired scope is genuinely unclear. Never produce a full-dialectic-shaped thesis/antithesis/synthesis from the orchestrator's single context without labeling it a correlated provisional sketch.
...
Any 2×2 readout must follow [Rao's cluster-first procedure and the rendering contract](reference/quadrant-diagrams.md#rao-sequence). Show the formation trace in order: typed phenomenology inventory; candidate four-cluster partitions with members, prototypes, overlaps, and outliers; candidate separating lines with chemistry and failure notes; label workshop with naming mode and alternatives; then every complete inline monospaced ASCII/box diagram. Each rendered map needs both named axes and poles, visible directions, four bounded evocatively labeled cells, and 1–2 relevant examples in every occupied cell or an explicit empty mark. A polished diagram without this trace—or a heading such as `axis A × axis B` followed by four labels, bullets, or a table—is not a complete 2×2 readout. Preserve an honest contraindication instead of forcing a diagram when the material does not support one.
...
- context-isolated committed positions, determinate negation, candidate comparison, validation, or recursion would change the inquiry enough to repay their cost;
- the user asks for a dialectic or names dialectic as the desired output.

Read [dialectic-workflow.md](reference/dialectic-workflow.md). Adopt the Walk, Field Trip, and Expedition record as Phase 1 substrate; ask only for gaps. Preserve the original question, goal, evidence types, loaded terms, current tensions, working question, actual execution traces, and lineage.

Inside the workflow, run the [phase-opening gate](reference/dialectic-workflow.md#phase-opening-gate) before every numbered phase. A completion gate proves that the prior phase ended; it never starts the next one. Never batch a phase merely because its instruments are scheduled.

## Electric Monk dialectic core

The dialectic workflow is an artificial belief system. Context-isolated agents carry committed positions so the user can inspect the structure of the contradiction without having to believe either side.
...
Use the strongest available agents for the dialectic workflow. Diverse models can help decorrelate Monks, but strong role and framing differences matter more than provider variety.

## The Completion Gate

This gate applies **inside the full dialectic workflow** at every phase and stage boundary. It does not apply merely because work is a Field Trip or Expedition.
...
5. **Gate.** Proceed only when all items are ✅. Any partial or missing item stops the apparatus.
...
Treat gardener work as background bookkeeping. Dispatch or resume the gardener, then continue every independent orchestrator task and user checkpoint. Do not wait for gardener startup, readiness, routine ingest, linking, or lint merely because the request was sent. Synchronize only when the next operation directly consumes a gardener-produced artifact or when every other completion-gate item is done and the gate still needs its ingest proof. Check the promised file or status first; wait only for that named dependency, not for general gardener idleness. Do not mention routine gardener state to the user; surface it only when it changes a reading, blocks a promised artifact, or stops a gate.
...
1. [Dialectic instrument map](reference/dialectic-instrument-map.md)
...
Map roles to the host’s subagent tools. Spawn Monks in separate fresh contexts and keep them blind to one another. Run tasks in parallel when they do not depend on one another and the host supports it. Run the gardener in the background: after spawning it or sending a handoff, return immediately to interview, reading, analysis, another agent, artifact preparation, or the user. Resume the gardener when possible; if its session is lost, re-ground it from the wiki on disk. For validation, resume the original Monk when possible, reinforcing its role; otherwise give a fresh validator the original position and only the candidate it must test.
