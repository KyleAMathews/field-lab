...
The skill runs **three agent roles**. **You (the orchestrator)** coordinate and reason. A persistent **gardener** owns the research wiki (`reference/dialectic-wiki.md`) — **spawn it before any research and keep it for the whole dialectic.** You never write the wiki, and **you never read raw research drafts**: research subagents write page drafts to a staging directory and return only paths (`reference/research-subagent-prompt.md`); you hand the paths to the gardener, which ingests them and assembles the curated artifacts you read — the **context briefing** and the **monk briefs**. **If you find yourself reading a research agent's returned prose, STOP** — that is the context-pollution failure this structure exists to prevent. (The interview material you gather yourself is fine; the prohibition is specifically on raw research-agent output.)
